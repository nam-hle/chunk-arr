...
var chunkBy_1 = require("../main/chunkBy");
describe('chunkBy', function () {
    ...
    describe('given an empty array', function () {
        it('should return an empty array', function () {
            chai_1.expect(chunkBy_1.chunkBy([], isEven)).to.be.deep.equal([]);
        });
    });
    describe('given an array with one element', function () {
        it('should return an array that contains one array with that element rega', function () {
            chai_1.expect(chunkBy_1.chunkBy([3], isEven)).to.be.deep.equal([[3]]);
            chai_1.expect(chunkBy_1.chunkBy([3], isOdd)).to.be.deep.equal([[3]]);
        });
    });
    describe("given a number array", function () {
        describe('given isEven func', function () {
            it('should chunk by modulo of each element ', function () {
                chai_1.expect(chunkBy_1.chunkBy(basicArray, isEven)).to.be.deep.equal([
                    [3, 1],
                    [4],
                    [1, 5, 9],
                    [2, 6]
        ...
        describe('given isSmallerThanFive func', function () {
            it('should chunk when the element smaller than five', function () {
                chai_1.expect(chunkBy_1.chunkBy(basicArray, function (number) { return number < 5; })).to.be.deep.equal([
                    [3, 1, 4, 1],
                    [5, 9],
                    [2],
                    [6]
    ...
    describe('given an array of objects', function () {
        it('should not copy the objects', function () {
            ...
            var result = chunkBy_1.chunkBy(users, function (_a) {
                var role = _a.role;
                return role === 'guest';
            });
    ...
    describe('describe given a function used to chunk by size', function () {
        it('should chunk array by size', function () {
            chai_1.expect(chunkBy_1.chunkBy([3, 1, 4, 1, 5, 9, 2, 6], function (_, index) { return Math.floor(index / 3); })).to.be.deep.equal([
                [3, 1, 4],
                [1, 5, 9],
                [2, 6]
            ]);
